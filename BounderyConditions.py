...
class BounderyConditions(abc.ABC):
    ...
    def handle_wall_collision(self,ball: Ball,wall_index: int):
        """
        :param wall_index: the wall_index of colliding wall, this value is returned automatically from "calc_collision_time_with_walls"
        """
        pass

    @staticmethod
    def calc_collision_time_with_wall_1d(ball_x,wall_x,ball_vel_x):
...
class RectangleBoundery_2D(BounderyConditions):
    ...
    def calc_collision_time_with_walls(self,ball: Ball) -> Tuple[float,int]:
        ...
        :return: a tuple: (collision time, index of colliding wall) the tuple's contents should be passed in this order to "handle_wall_collision"
        """
        ret_1 = self.calc_collision_time_with_wall_1d(ball.location[0],self.wall_x_0,ball.velocity[0]) , self.wall_x_0_index
        ret_2 = self.calc_collision_time_with_wall_1d(ball.location[0],self.wall_x_1,ball.velocity[0]) , self.wall_x_1_index
        ret_3 = self.calc_collision_time_with_wall_1d(ball.location[1],self.wall_y_0,ball.velocity[1]) , self.wall_y_0_index
        ret_4 = self.calc_collision_time_with_wall_1d(ball.location[1],self.wall_y_1,ball.velocity[1]) , self.wall_y_1_index
